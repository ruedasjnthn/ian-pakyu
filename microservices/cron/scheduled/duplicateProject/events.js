...
async function runDuplicateProjectJobsEvents() {
  try {
    loggerInfo('runDuplicateProjectJobsEvents', jobsIdsProcessing)
    ...
    const jobs = await DuplicateProjectJob
      .find(
        { status: DuplicateProjectJobStatusType.READY_TO_DUPLICATE_EVENTS, },
        'projectId status'
      )
    ...
    for (const job of jobs) {
      ...
      const mutationObject = {
        mutation: gql`
              mutation duplicateProjectEvents(
                $projectId: ID!, 
                $duplicateProjectJobId: ID!
              ) {
                duplicateProjectEvents(
                  projectId: $projectId
      ...
      try {
        ...
        loggerError('duplicateProjectEvents', { e })
    ...
    jobsIdsProcessing = []
    loggerError('runDuplicateProjectJobsEvents', { e })
  }
}
