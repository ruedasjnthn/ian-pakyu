...
async function runDuplicateProjectJobsCustomFields() {
  try {
    loggerInfo('runDuplicateProjectJobsCustomFields', jobsIdsProcessing)
    ...
    const jobs = await DuplicateProjectJob
      .find(
        { status: DuplicateProjectJobStatusType.READY_TO_DUPLICATE_CUSTOM_FIELDS, },
        'projectId status'
      )
    ...
    for (const job of jobs) {
      ...
      const mutationObject = {
        mutation: gql`
              mutation duplicateProjectCustomFields(
                $projectId: ID!, 
                $duplicateProjectJobId: ID!
              ) {
                duplicateProjectCustomFields(
                  projectId: $projectId
      ...
      try {
        ...
        loggerError('duplicateProjectCustomFields', { e })
    ...
    jobsIdsProcessing = []
    loggerError('runDuplicateProjectJobsCustomFields', { e })
  }
}
