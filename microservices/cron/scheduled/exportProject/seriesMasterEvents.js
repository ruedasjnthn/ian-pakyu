...
async function runExportProjectJobsSeriesMasterEvents() {
  try {
    loggerInfo('runExportProjectJobsSeriesMasterEvents', jobsIdsProcessing)
    ...
    const jobs = await ExportProjectJob
      .find(
        { status: ExportProjectJobStatusType.READY_TO_EXPORT_SERIES_MASTER_EVENTS, },
        'projectId status'
      )
    ...
    for (const job of jobs) {
      ...
      const mutationObject = {
        mutation: gql`
              mutation exportProjectSeriesMasterEvents(
                $projectId: ID!, 
                $exportProjectJobId: ID!
              ) {
                exportProjectSeriesMasterEvents(
                  projectId: $projectId
      ...
      try {
        ...
        loggerError('exportProjectSeriesMasterEvents', { e })
    ...
    jobsIdsProcessing = []
    loggerError('runExportProjectJobsSeriesMasterEvents', { e })
  }
}
