...
async function runExportProjectJobsEvents() {
  try {
    loggerInfo('runExportProjectJobsEvents', jobsIdsProcessing)
    ...
    const jobs = await ExportProjectJob
      .find(
        { status: ExportProjectJobStatusType.READY_TO_EXPORT_EVENTS, },
        'projectId status'
      )
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        loggerError('exportProjectEvents', { e })
    ...
    jobsIdsProcessing = []
    loggerError('runExportProjectJobsEvents', { e })
  }
}
