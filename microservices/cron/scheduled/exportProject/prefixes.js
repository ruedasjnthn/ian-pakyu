...
async function runExportProjectJobsPrefixes() {
  try {
    loggerInfo('runExportProjectJobsPrefixes', jobsIdsProcessing)
    ...
    const jobs = await ExportProjectJob
      .find(
        { status: ExportProjectJobStatusType.READY_TO_EXPORT_PROJECT_PREFIXES, },
        'projectId status'
      )
    ...
    for (const job of jobs) {
      ...
      const mutationObject = {
        mutation: gql`
              mutation exportProjectPrefixes(
                $projectId: ID!, 
                $exportProjectJobId: ID!
              ) {
                exportProjectPrefixes(
                  projectId: $projectId
      ...
      try {
        ...
        loggerError('exportProjectPrefixes', { e })
    ...
    jobsIdsProcessing = []
    loggerError('runExportProjectJobsPrefixes', { e })
  }
}
