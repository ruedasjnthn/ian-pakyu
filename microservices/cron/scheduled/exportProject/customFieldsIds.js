...
async function runExportProjectJobsInitCustomFieldsIds() {
  try {
    loggerInfo('runExportProjectJobsInitCustomFieldsIds', jobsIdsProcessing)
    ...
    const jobs = await ExportProjectJob
      .find(
        { status: ExportProjectJobStatusType.READY_TO_INIT_CUSTOM_FIELDS_IDS, },
        'projectId status'
      )
    ...
    for (const job of jobs) {
      ...
      const mutationObject = {
        mutation: gql`
              mutation exportProjectInitCustomFieldsIds(
                $projectId: ID!, 
                $exportProjectJobId: ID!
              ) {
                exportProjectInitCustomFieldsIds(
                  projectId: $projectId
      ...
      try {
        ...
        loggerError('exportProjectInitCustomFieldsIds', { e })
