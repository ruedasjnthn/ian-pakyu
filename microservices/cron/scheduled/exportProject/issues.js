...
async function runExportProjectJobsIssues() {
  try {
    loggerInfo('runExportProjectJobsIssues', jobsIdsProcessing)
    ...
    const jobs = await ExportProjectJob
      .find(
        { status: ExportProjectJobStatusType.READY_TO_EXPORT_ISSUES, },
        'projectId status'
      )
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        loggerError('exportProjectIssues', { e })
    ...
    jobsIdsProcessing = []
    loggerError('runExportProjectJobsIssues', { e })
  }
}
