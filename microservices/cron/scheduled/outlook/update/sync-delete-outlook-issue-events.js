...
// run sync  events
async function runSyncDeleteSyncedOutlookIssueEventsOutlookJobs() {
  try {
    loggerInfo('runSyncDeleteSyncedOutlookIssueEventsOutlookJobs', onSyncProcess)

    const outlookSync = await OutlookSync.findOne(
      {
        _id: { $nin: onSyncProcess },
        status: OutlookCalendarSyncStatus.READY_TO_DELETE_OUTLOOK_ISSUE_EVENTS
      },
    ...
    if (canExecuteSync) {
      ...
      const gqlRequest = async () => {
        ...
        const mutationObject = {
          mutation: gql`
              mutation deleteSyncedOutlookIssueEvents(
                $projectId: ID!, 
                $outlookSyncId: ID!
              ) {
        ...
        let outlookSyncId = data && data.deleteSyncedOutlookIssueEvents
      ...
      //   gqlRequest,
      //   onError,
      //   funcId: 'runSyncDeleteSyncedOutlookIssueEventsOutlookJobs'
      // })
      ...
      await handleCalSyncOnError({
        ...
        funcId: 'runSyncDeleteSyncedOutlookIssueEventsOutlookJobs',
        errorStatus: OutlookCalendarSyncStatus.FAILED_TO_DELETE_OUTLOOK_ISSUE_EVENTS,
        retryStatus: OutlookCalendarSyncStatus.READY_TO_DELETE_OUTLOOK_ISSUE_EVENTS,
      })
    ...
    onSyncProcess = []
    loggerError('runSyncDeleteSyncedOutlookIssueEventsOutlookJobs', { e })
  }
}
