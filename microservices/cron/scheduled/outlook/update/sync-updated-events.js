...
// run sync  events
async function runSyncUpdatedEventsOutlookJobs() {
  try {
    loggerInfo('runSyncUpdatedEventsOutlookJobs', onSyncProcess)

    const outlookSync = await OutlookSync.findOne(
      {
        _id: { $nin: onSyncProcess },
        status: OutlookCalendarSyncStatus.READY_TO_SYNC_UPDATED_EVENTS
      },
    ...
    if (canExecuteSync) {
      ...
      const gqlRequest = async () => {
        ...
        let outlookSyncId = data && data.syncUpdatedEvents
      ...
      //   gqlRequest,
      //   onError,
      //   funcId: 'runSyncUpdatedEventsOutlookJobs'
      // })
      ...
      await handleCalSyncOnError({
        ...
        funcId: 'runSyncUpdatedEventsOutlookJobs',
        errorStatus: OutlookCalendarSyncStatus.FAILED_TO_SYNC_UPDATED_EVENTS,
        retryStatus: OutlookCalendarSyncStatus.READY_TO_SYNC_UPDATED_EVENTS,
      })
    ...
    onSyncProcess = []
    loggerError('runSyncUpdatedEventsOutlookJobs', { e })
  }
}
