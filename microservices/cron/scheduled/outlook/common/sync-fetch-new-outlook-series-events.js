...
// run sync  events
async function runSyncFetchNewOutlookSeriesEventsOutlookJobs() {
  try {
    loggerInfo('runSyncFetchNewOutlookSeriesEventsOutlookJobs', onSyncProcess)

    const outlookSync = await OutlookSync.findOne(
      {
        _id: { $nin: onSyncProcess },
        status: OutlookCalendarSyncStatus.READY_TO_FETCH_NEW_OUTLOOK_SERIES_EVENTS
      },
    ...
    if (canExecuteSync) {
      ...
      const gqlRequest = async () => {
        ...
        const mutationObject = {
          mutation: gql`
                mutation fetchNewOutlookSeriesEvents(
                  $projectId: ID!, 
                  $outlookSyncId: ID!
                ) {
        ...
        let outlookSyncId = data && data.fetchNewOutlookSeriesEvents
      ...
      await handleCalSyncOnError({
        ...
        funcId: 'runSyncFetchNewOutlookSeriesEventsOutlookJobs',
        errorStatus: OutlookCalendarSyncStatus.FAILED_TO_FETCH_NEW_OUTLOOK_SERIES_EVENTS,
        retryStatus: OutlookCalendarSyncStatus.READY_TO_FETCH_NEW_OUTLOOK_SERIES_EVENTS,
      })
    }

  } catch (e) {
    onSyncProcess = []
    loggerError('runSyncFetchNewOutlookSeriesEventsOutlookJobs', { e })
  }
}
