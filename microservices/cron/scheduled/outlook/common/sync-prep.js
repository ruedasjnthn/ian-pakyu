...
async function runSyncPrepInitOutlookJobs() {
  try {
    loggerInfo('runSyncPrepInitOutlookJobs', onSyncProcess)
    ...
    if (canExecuteSync) {
      ...
      const gqlRequest = async () => {
        ...
        let outlookSyncId = data && data.prepSyncInit
      ...
      //   gqlRequest,
      //   onError,
      //   funcId: 'runSyncPrepInitOutlookJobs'
      // })
      ...
      await handleCalSyncOnError({
        ...
        funcId: 'runSyncPrepInitOutlookJobs',
        errorStatus: OutlookCalendarSyncStatus.FAILED_TO_PREP_INIT,
        retryStatus: OutlookCalendarSyncStatus.READY_TO_PREP_INIT,
      })
    }
    ...
    //     )
    //   }
    //   loggerError('ERROR: prepSyncInit,', { e })
    // }

  } catch (e) {
    onSyncProcess = []
    loggerError('runSyncPrepInitOutlookJobs', { e })
  }
}
