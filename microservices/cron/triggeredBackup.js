  ...
  await Promise.all(
    cols.map(async (collection) => {
      ...
      if (records && records.length > 0) {
        fs.writeFileSync(
          `${folder}${PATH_SEP}${collection.toLowerCase()}.json`,
...
async function zipDirectory(sourceDir, outPath, fn, isMove) {
  ...
  archive
    .directory(sourceDir, loggerError(`Error zip :: `))
    .pipe(stream);
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      if (createProjectFolder(folder)) {
        ...
        fs.writeFileSync(
          `${folder}${PATH_SEP}col_projects.json`,
...
async function runTriggerBackup() {
  ...
  if (createProjectFolder(`${dir}`) && createProjectFolder(`${zip_dir}`)) {
    ...
    const ftpDir = `${new Date()
      .toISOString()
      .slice(0, 10)}_${new Date().getTime()}`;

    //only backup projects triggered via app
    const backupEnabledProjects = await connection
      .collection('col_BackupEnabledProjects')
      .find({ isEnabled: true, isBackedUp: false })
      .toArray();
    const backupProjectIds = backupEnabledProjects.map(({ projectId }) =>
      mongoose.Types.ObjectId(projectId),
    );
    loggerInfo({ backupProjectIds });

    let projects = await connection
      .collection('col_Projects')
      .find({ _id: { $in: backupProjectIds } })
      .toArray();
    ...
    Promise.all(
      projects.map(async (project) => {
        exportProject(project, connection, ftpDir).catch((err) =>
          loggerError(`Error Exporting :: `, { err }),
        );
      }),
    );
...
async function ftpUpload(filePath, ftpDir) {
  ...
  try {
    ...
    await client.uploadFromDir(`${filePath}`);
  } catch (err) {
    loggerError({ err });
  }
    ...
    minioClient.putObject(
      ...
      function (err, objInfo) {
        ...
        try {
          ...
          loggerInfo(`Error saving to col_Backup:: `, { error });
        }
      },
    );
  });
...
module.exports = runTriggerBackup;
